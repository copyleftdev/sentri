...
fn debug_auto_domain_xml() -> Result<()> {
    ...
    match parser.parse_federation_response(test_xml) {
        Ok(federation_info) => {
            println!(
                "DEBUG: XmlParser found domains: {:?}",
                federation_info.domains
            );

            // Check if we found the domain
            assert_eq!(
                federation_info.domains.len(),
                1,
                "Expected to find 1 domain"
            );
            assert!(
                federation_info.domains.contains(&"contoso.com".to_string()),
