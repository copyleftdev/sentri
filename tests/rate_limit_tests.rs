...
async fn test_rate_limiter_permits() -> Result<()> {
    ...
    assert!(elapsed.as_millis() >= 800, "Did not wait for rate limit: {:?}", elapsed);
...
async fn test_rate_limiter_update_config() -> Result<()> {
    ...
    assert!(acquire_result.is_err(), "Should have timed out waiting for permit");
    ...
    assert!(permit_result.is_ok(), "Should have received a permit after update");
