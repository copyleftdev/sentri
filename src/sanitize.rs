...
pub fn sanitize_domain_result(result: &DomainResult) -> DomainResult {
    ...
    DomainResult {
        ...
        federated_domains: result.federated_domains.iter()
...
fn sanitize_error(error: &str) -> String {
    ...
    let path_pattern = regex::Regex::new(r"(/[a-zA-Z0-9_\-\.]+)+").unwrap_or_else(|_| regex::Regex::new(r"").unwrap());
    path_pattern.replace_all(&sanitized, "[REDACTED_PATH]").to_string()
}
...
mod tests {
    ...
    fn test_sanitize_domain() {
        ...
        assert_eq!(sanitize_domain("<script>alert(1)</script>"), "&lt;script&gt;alert(1)&lt;/script&gt;");
        assert_eq!(sanitize_domain("domain.com\n\rinjection"), "domain.cominjection");
    }
    ...
    fn test_sanitize_domain_result() {
        ...
        assert_eq!(sanitized.tenant, Some("tenant&lt;img src=x&gt;".to_string()));
        assert_eq!(sanitized.federated_domains, vec!["a.com".to_string(), "b.com".to_string()]);
        assert_eq!(sanitized.mdi_instance, Some("instance.atp.azure.com".to_string()));
        assert_eq!(sanitized.error, Some("Failed at [REDACTED_PATH]".to_string()));
    }
}
